...
| Store model | src/decapod/core/store.rs, SQLite subsystems, RPC store operations | todo, knowledge, aptitude, federation, policy, health, context, tests | Store kind, state owner, scope, mutation authorization, entity envelopes |
| Testing and validation | test harnesses, src/decapod/core/validate.rs, interfaces/TESTING | CI, local agents, release checks | Command identity, evidence threshold, skipped/failed/blocker distinction |
