...
pub fn ensure_execute_ready(
    ...
    if let Some(active) = plan
        .phases
        .iter()
        .find(|phase| phase.entered && !phase.completed)
    {
        let (entry_ok, entry_failures) = check_phase_entry_gates(input.project_root, &active.id)?;
        if !entry_ok {
            return Err(marker_error(
                ...
                Some(json!({ "phase": active.id, "failures": entry_failures })),
            ));
        }
    } else if plan.phases.iter().any(|phase| !phase.completed) {
        return Err(marker_error(
            "PHASE_REQUIRED",
            "Execution is blocked until the next declared phase is entered.",
            Some(
                json!({ "next_phase": plan.phases.iter().find(|phase| !phase.completed).map(|phase| phase.id.clone()) }),
            ),
        ));
    }
...
fn validate_phase_contract(phases: &[Phase]) -> Result<(), error::DecapodError> {
    ...
    for phase in phases {
        ...
        if phase.completed && !phase.entered {
            return Err(marker_error(
                "INVALID_PHASE_CONTRACT",
                &format!("Phase '{}' is completed but was never entered", phase.id),
                None,
            ));
        }
    }
