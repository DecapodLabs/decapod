...
fn validate_project_specs_docs(
    ...
    if let Some(manifest) = manifest {
        ...
        if template_is_current {
            ...
            warn(
                &format!(
                    "STALE_SPECS_TEMPLATE: Project specs use template version '{}' while current scaffolding is '{}'; living spec content remains authoritative.",
        ...
        if current_repo_fp == manifest.repo_signal_fingerprint {
            ...
            pass(
                "Project specs repo-signal fingerprint refreshed after codebase re-evaluation",
                ctx,
            );
        } else if refresh_specs {
            info(
...
#[cfg(test)]
#[path = "../../../tests/unit/core/validate_tests.rs"]
mod tests;
