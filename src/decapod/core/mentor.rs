    ...
    fn get_container_candidates(&self) -> Result<Vec<Obligation>, DecapodError> {
            ...
            if path.exists() {
                ...
                obligations.push(Obligation {
                    ...
                    why_short: if is_workspace_seed {
                        ".decapod/managed/Dockerfile.decapod is Decapod's internal workspace container; the root Dockerfile must package the project application or microservice according to human intent.".to_string()
                    } else {
        ...
        if !found_dockerfile {
            obligations.push(Obligation {
                ...
                why_short: ".decapod/managed/Dockerfile.decapod is Decapod's internal workspace container; create a root Dockerfile that packages the project application or microservice according to human intent."
                    .to_string(),
                evidence: Evidence {
                    source: "workspace".to_string(),
...
#[cfg(test)]
#[path = "../../../tests/unit/core/mentor_tests.rs"]
mod tests;
