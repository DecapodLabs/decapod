...
pub fn prepare_generated_container_profile(repo: &Path) -> Result<PathBuf, error::DecapodError> {
    ...
    let managed_dir = repo.join(".decapod").join("managed");
    fs::create_dir_all(&managed_dir).map_err(error::DecapodError::IoError)?;
    let dockerfile = managed_dir.join("Dockerfile");
...
pub fn schema() -> serde_json::Value {
    ...
    json!({
        ...
        "profiles": {
            ...
            "alpine": "local build from .decapod/managed/Dockerfile (Decapod workspace seed + detected project dependencies + current decapod binary)"
        },
        "components": {
