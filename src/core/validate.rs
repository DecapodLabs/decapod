    ...
    if has_golden {
            ...
            std::fs::read_to_string(golden_v1_dir.join("scope_record_hash.txt"))
        {
            if actual_hash.trim() != expected_scope_hash {
                fail(
                    &format!("STATE_COMMIT v1 scope_record_hash changed! Expected {}, got {}. This requires a SPEC_VERSION bump to v2.", expected_scope_hash, actual_hash.trim()),
                    fail_count,
                );
            }
        }
            ...
            std::fs::read_to_string(golden_v1_dir.join("state_commit_root.txt"))
        {
            if actual_root.trim() != expected_root {
                fail(
                    &format!("STATE_COMMIT v1 state_commit_root changed! Expected {}, got {}. This requires a SPEC_VERSION bump to v2.", expected_root, actual_root.trim()),
                    fail_count,
                );
            }
        }
