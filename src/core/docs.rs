...
pub fn get_bindings(_repo_root: &Path) -> Bindings {
    ...
    ops.insert(
        ...
        "core/DECAPOD.json#workspaces".to_string(),
    );
    ops.insert(
    ...
    ops.insert(
        "validate".to_string(),
        "core/DECAPOD.json#validation".to_string(),
    );

    let mut paths = std::collections::HashMap::new();
    paths.insert("rpc".to_string(), "interfaces/CONTROL_PLANE.json".to_string());

    let mut tags = std::collections::HashMap::new();
    tags.insert("security".to_string(), "specs/SECURITY.json".to_string());
...
fn get_mandate_by_id(repo_root: &Path, id: &str) -> Option<Mandate> {
    match id {
        ...
        "mandatory-todo" => Some(Mandate {
            ...
            fragment: get_fragment(repo_root, "core/DECAPOD.json", Some("Subsystems"))?, // We'll link to todo section
