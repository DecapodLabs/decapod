...
fn render_timeline(store: &Store, format: &str, limit: usize) -> Result<(), DecapodError> {
    ...
    if format == "json" {
        ...
        println!("Use `decapod govern flight-recorder transcript` for detailed output.");
    }
...
fn read_events(path: &PathBuf, limit: usize) -> Result<Vec<TimelineEvent>, DecapodError> {
    let file = File::open(path).map_err(|e| DecapodError::IoError(e))?;
    let reader = BufReader::new(file);
    let mut events = Vec::new();

    for line in reader.lines() {
        let line = line.map_err(|e| DecapodError::IoError(e))?;
...
fn render_transcript(
    ...
    if let Some(path) = output_path {
        std::fs::write(path, &md).map_err(|e| DecapodError::IoError(e))?;
