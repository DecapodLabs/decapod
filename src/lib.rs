...
fn should_auto_clock_in(command: &Command) -> bool {
    match command {
        ...
        Command::Version | Command::Init(_) | Command::Setup(_) | Command::Session(_) | Command::StateCommit(_) => false,
        _ => true,
    }
}
...
fn run_state_commit_command(
    ...
    match cli.command {
        StateCommitCommand::Prove { base, head, output } => {
            ...
            // Write output
            std::fs::write(&output, &result.scope_record_bytes).map_err(|e| {
                error::DecapodError::IoError(e)
            })?;
        ...
        StateCommitCommand::Verify { scope_record, expected_root } => {
            // Read scope record
            let cbor_bytes = std::fs::read(&scope_record).map_err(|e| {
                error::DecapodError::IoError(e)
            })?;
        ...
        StateCommitCommand::Explain { scope_record } => {
            ...
            if let Some(version_pos) = content.find("state_commit.") {
                if let Some(end_pos) = content[version_pos..].find('\0') {
                    println!("  algo_version: {}", &content[version_pos..version_pos+end_pos]);
                }
            }
