...
fn maintain_generated_dockerfile_contract(current: &str, expected: &str) -> Option<String> {
    let expected_image = expected
        .lines()
        .find(|line| line.starts_with("ARG DECAPOD_IMAGE="))?;
    ...
    let current_has_version = current
        .lines()
        .any(|line| line.starts_with("ARG DECAPOD_VERSION="));
    let mut out = Vec::new();
    for line in current.lines() {
        if line.starts_with("ARG DECAPOD_IMAGE=") {
            if line != expected_image {
                changed = true;
            }
            out.push(expected_image.to_string());
        } else if line.starts_with("ARG DECAPOD_VERSION=") {
            if saw_version {
                changed = true;
                continue;
            }
...
#[cfg(test)]
mod generated_dockerfile_heal_tests {
...
fn run_validate_command(
    ...
    if let Some(action) = heal_generated_dockerfile(workspace_root)? {
        heal_actions.push(action);
    }
    ...
    match plan_cli.command {
        ...
        PlanCommand::Phase(phase_cli) => match phase_cli.command {
            PlanPhaseCommand::Add {
                ...
                exit_required_artifacts,
                exit_required_verified_todos,
                remediation,
            } => {
                let entry_gates = required_artifacts
                    ...
                    .collect();
                let exit_gates = exit_required_artifacts
                    .into_iter()
                    .map(|path| plan_governance::PhaseGate::ArtifactExists { path })
                    .chain(
                        exit_required_verified_todos
            ...
            PlanPhaseCommand::Complete { id } => {
                let plan = plan_governance::complete_phase(project_root, &project_store.root, &id)?;
                println!("{}", serde_json::to_string_pretty(&plan).unwrap());
            }
        },
    }
            ...
            let config = if branch.is_some() || container {
                Some(workspace::WorkspaceConfig {
                    ...
                    base_image: if container {
                        Some("rust:1.96.1-slim".to_string())
                    } else {
                        None
                    },
                })
