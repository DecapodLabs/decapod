...
| Check | Failure Mode |
|-------|--------------|
| Phase contracts: unique IDs, no duplicate declarations | Fail |
| Phase lifecycle: pending → active → completed | Fail |
| Entry gates evaluated on phase enter | Fail |
| Exit gates evaluated on phase complete | Fail |
| Active phase required when phases declared | Fail |
| Final plan state requires all phases completed | Fail |
| Artifact gates: existence proof only (not correctness) | N/A |
| Verified-TODO gates: require Decapod verification evidence | Fail |
| TODO done bypass prevention for phase-bearing plans | Fail |

Phase gate markers:
- `PHASE_GATE_FAILED` - entry or exit gate evaluation failed
- `INVALID_PHASE_TRANSITION` - invalid phase state transition attempted
- `INVALID_PHASE_CONTRACT` - malformed phase contract
- `PHASE_LOCKED` - concurrent phase mutation blocked
