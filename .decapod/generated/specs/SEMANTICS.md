...
stateDiagram-v2
    ...
    Pending --> Active: phase enter + entry gates pass
    Active --> Completed: phase complete + exit gates pass
    Completed --> Pending: next phase declared
    Completed --> [*]: final phase complete
```
- Phases are ordered; only one phase may be active at a time
- Plans without phases are backward-compatible (no phase enforcement)
- Plans with phases cannot reach DONE until every phase is completed
...
| Phase gates enforce ordered completion | Governance | `ensure_execute_ready` blocks without active phase when phases declared |
| Artifact gates prove existence only | Governance | Verified-TODO gates require Decapod verification evidence |
