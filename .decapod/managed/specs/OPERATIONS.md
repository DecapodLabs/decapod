...
| Release plan | `Release / plan` failed on release PRs #1353 and #1355 in Buildkite while the native GitHub Actions plan passed; the imported `run` block was first interpolated before runtime, then the installer path was not guaranteed to survive the non-login step boundary | Run cargo-dist planning only for `release-plz-*` PRs and explicit tags, keep runtime selection in `.github/scripts/release-plan.sh`, restore `${CARGO_HOME:-$HOME/.cargo}/bin` before invoking cargo-dist, treat an explicit tag as the only publishing context, default all other invocations to a non-publishing plan, and emit manifest/tag/publishing outputs from one shell step. |
