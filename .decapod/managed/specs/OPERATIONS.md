...
Initialization may also store an explicitly supplied key in the machine-local
`~/.local/share/decapod/secrets.json` file with restrictive permissions; it is
never copied into repository state. `decapod init --refresh` reopens the
initialization questionnaire for an existing project, preserves current
settings as defaults, and can enable or disable optional features such as Jev
without destructive reset behavior.

This repository opts into `jev` in `.decapod/config.toml` for advisory
trajectory observations. The opt-in does not change Decapod authority, and a
missing machine-local credential leaves the observation unavailable rather
than authorizing work.
