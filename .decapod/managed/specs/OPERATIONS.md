...
| Release plan | `Release / plan` failed on release PR #1353 in Buildkite while the native GitHub Actions plan passed; the imported `run` block was subject to Buildkite pipeline interpolation, and the hosted job did not expose a usable event/PR marker to the runtime script | Run cargo-dist planning only for `release-plz-*` PRs and explicit tags, keep the runtime selection in the checked-in `.github/scripts/release-plan.sh` script so the workflow command contains no interpolated shell variables, treat an explicit tag (`GITHUB_REF_TYPE=tag` or `BUILDKITE_TAG`) as the only publishing context, default all other invocations to a non-publishing plan, and emit manifest/tag/publishing outputs from one shell step. |
