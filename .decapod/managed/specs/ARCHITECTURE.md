...
- Backend binding establishes Dactyl's process-scoped ambient inputs before a route-scoped connection opens: `DATASTORE=sqlite|neon`, the opaque `DATASTORE_ROUTE`, and cloud-only `DATASTORE_TOKEN`. Dactyl parses those variables with `DatastoreRoute::from_env`; Decapod does not add a provider selector to execution calls, and local binding removes any stale cloud token. The bind-and-open step is serialized because the environment is process-global, while the resulting connection retains its own route.
