...
| `core::dactyl::DactylBridge` | Backend route, access mode, and optional opaque bearer | Dactyl v0.8.2 operation/context contract | Establishes `DATASTORE`, `DATASTORE_ROUTE`, and cloud-only `DATASTORE_TOKEN`, then resolves the physical route through Dactyl `from_env()`; local binding clears stale cloud credentials, cloud construction fails closed without a bearer, and opaque context forwarding remains unchanged |
| `core::dactyl_todo::DactylTodoStore` | Cloud `TodoStore` command boundary plus versioned storage context | Dactyl `/query` and `/batch` handlers | Lists canonical task columns; add/claim/complete use conditional SQL write plus row observation in one batch; zero affected rows are conflicts; no per-query backend, tenant, provider, or repository input is accepted |
