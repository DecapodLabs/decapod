...
| `DbBroker::with_transaction` | Decapod domain mutation | Dactyl atomic operation | Commits state and its canonical event together; rolls both back on failure; no SQLite transaction handle crosses the boundary |
...
| `core::dactyl::DactylBridge` | Backend route, access mode, and optional opaque bearer | Dactyl v0.7.0 / Dactyl #77 contract | Provides explicit reads, writes, atomic batches, portable schema inspection, access-mode enforcement, Decapod-normalized errors, and the canonical file-backed Dactyl-snapshot route; legacy SQLite conversion is delegated to Dactyl and cloud construction fails closed without a bearer |
...
| `todo::update_status` | Decapod lifecycle caller | Target canonical Dactyl operation batch; current closure is transitional | Requires expected status and revision; returns `ok`, `not_found`, or `conflict` and emits an event only after the conditional update wins; direct SQLite callers remain migration work |
| `todo` lease/ownership mutations | Decapod coordination caller | Target canonical Dactyl operation batch; current closure is transitional | Claim, yield, handoff, owner, heartbeat, cleanup, and expertise state plus their events must share one Dactyl atomic batch |
| `events::append_on_conn` | Transitional SQLite-only domain helper | Must be replaced for canonical runtime | Replaying the same event identity is idempotent; divergent content fails with `EVENT_ID_CONFLICT`; the Dactyl replacement must preserve unique stream sequencing and atomicity |
...
| `core::events::{query,latest,exists,actors}` | Semantic stream and bounded filter/limit | Target canonical Dactyl event observations; current implementation is transitional SQLite | Unknown stream, malformed canonical payload, unsupported physical operation, or pre-import SQLite format must be a typed failure after cutover |
| startup reconciliation | Unproven legacy JSONL or a legacy SQLite source | Dactyl-imported canonical rows plus durable retirement receipts | Dactyl owns SQLite inspection/conversion and atomic replacement; fresh malformed input or same-ID/different-event conflict stops migration; previously proven inputs are never reinterpreted as live authority |
