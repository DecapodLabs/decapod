...
| Context capsules | Current run | Ignored runtime state; cleanup is safe after the run because the tracked trajectory and Git history carry the durable record |
| Validation receipt | Per commit | Tracked successful receipt; overwritten by `validate`, with prior values retained in Git history |
...
|----------|------|--------------|
| Validation report | `.decapod/managed/artifacts/provenance/validation_report.json` | `validate` (ignored current-run output) |
| Proof manifest | `.decapod/managed/artifacts/provenance/proof_manifest.json` | `proof run` (ignored current-run output) |
| Artifact manifest | `.decapod/managed/artifacts/provenance/artifact_manifest.json` | `workspace publish` (ignored current-run output) |
...
| Assurance attestations | `.decapod/managed/assurance_attestations.jsonl` | `assurance.evaluate` |
...
- Repository signal fingerprint: `28274fef84e8b23a180a08018f9e29a810383129f28d1985cf3f06dfa4314cca`
- Significant implementation surfaces: `.github/` (8 files), `Cargo.lock/` (1 files), `Cargo.toml/` (1 files), `README.md/` (1 files), `docs/` (1 files), `src/` (95 files), `tests/` (4 files)
