...
| claim.memory.capture.invocation_auditable | Implicit memory capture is valid only through explicit invocation-scoped control-plane surfaces and must emit auditable events. | `interfaces/CONTROL_PLANE.md` | partially_enforced | `decapod validate` (Memory Event Integrity Gate + schema marker checks) | Capture can be automatic only after explicit per-store enablement. |
| claim.memory.temporal.as_of_respected | Retrievals using `as_of` must exclude entries with `created_ts` newer than `as_of`. | `interfaces/MEMORY_SCHEMA.md` | partially_enforced | `knowledge.search` temporal filters + `decapod validate` schema markers | Current implementation binds `as_of` to `created_ts` semantics. |
| claim.memory.decay.prune_audited | Decay/prune runs must be deterministic for declared policy inputs and recorded as append-only events. | `interfaces/MEMORY_SCHEMA.md` | partially_enforced | `knowledge.decay` event log + `decapod validate` decay event checks | Determinism is scoped to `(policy, as_of, store)` inputs. |
| claim.knowledge.merge.no_duplicate_active | For a given `(merge_key, scope)`, knowledge must not contain multiple active entries. | `interfaces/KNOWLEDGE_SCHEMA.md` | enforced | DB unique index + `decapod validate` duplicate-active check | Conflict policy must be explicit (`merge|supersede|reject`). |
| claim.memory.roi.retrieval_event_logged | Each explicit retrieval feedback submission must persist outcome metadata for ROI analysis. | `interfaces/MEMORY_SCHEMA.md` | enforced | `knowledge retrieval-log` command + `decapod validate` retrieval-event shape checks | This claim applies to submitted feedback, not all retrievals. |
| claim.memory.redaction.pointerization_required | Persisted memory/retrieval artifacts must redact or pointerize secret-bearing content. | `specs/SECURITY.md` | partially_enforced | `decapod validate` secret-pattern checks on retrieval event logs | Pattern-based gate; does not claim perfect semantic redaction. |
