...
| claim.architecture.artifact_required_for_governed_execution | Governed execution requires architecture artifact `<repo>/.decapod/governance/architecture.md` with mandatory sections. | `interfaces/ARCHITECTURE_FOUNDATIONS.md` | enforced | `decapod govern plan check-execute` + `decapod validate` plan-governed gate + `tests/plan_governed_execution.rs` | Missing/incomplete artifact returns typed `NEEDS_HUMAN_INPUT`. |
| claim.architecture.intent_to_design_traceability | Architecture artifact MUST map intent to system design, invariants, tradeoffs, verification, and rollout operations. | `interfaces/ARCHITECTURE_FOUNDATIONS.md` | enforced | `decapod govern plan init` artifact generation + section validation gate | Ensures user intent is translated into architecture decisions before execution. |
