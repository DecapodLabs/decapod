...
| claim.knowledge.provenance_required | Every procedural memory entry must cite evidence (commit, PR, doc, test, or transcript). | `interfaces/KNOWLEDGE_STORE.md` | enforced | `decapod knowledge add` requires --evidence | Rejects entries without provenance. |
| claim.knowledge.directional_flow | Episodic observations cannot flow directly into procedural/semantic memory. Must use explicit promotion artifact + human approval. | `interfaces/KNOWLEDGE_STORE.md` | enforced | `decapod knowledge promote` requires approval artifact | Blocks direct friction→procedural writes. |
| claim.knowledge.versioned_schema | Knowledge store uses versioned schemas. No breaking changes without migration path. | `interfaces/KNOWLEDGE_STORE.md` | enforced | Schema version in each entry + vN/ directories | Readers never break on writes. |
