...
- Links SHOULD be repo-relative paths in backticks (e.g. `core/DECAPOD.md`).
- `core/DECAPOD.md` MUST reach every canonical doc via the `## Links` graph (reachability) (claim: claim.doc.decapod_reaches_all_canonical).
- Constitution hop constraint (intended invariant):
  - Every Constitution doc with `**Binding:** Yes` SHOULD be linked directly from `core/DECAPOD.md` (no buried law).
- Interfaces hop constraint (intended invariant):
  - Every Interfaces doc with `**Binding:** Yes` SHOULD be reachable from `core/DECAPOD.md` within 2 hops (directly or via a single router doc).
...
- If two canonical binding docs appear to disagree, the system is in an invalid state.
  - Resolution is not interpretation; resolution is amendment (see `specs/AMENDMENTS.md`).
...
- be registered in `interfaces/CLAIMS.md`
