...
- If two canonical binding docs appear to disagree, the system is in an invalid state.
  - Resolution is not interpretation; resolution is amendment (see `embedded/specs/AMENDMENTS.md`).
...
This matrix defines which canonical doc owns which type of decision. If you need to change a decision, amend the owner doc (see `embedded/specs/AMENDMENTS.md`).
