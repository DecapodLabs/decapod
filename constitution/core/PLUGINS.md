...
- **todo** ⭐: Backlog management with audit trail. **See: `plugins/TODO.md`** (CLI: `decapod todo`)
...
- **teammate** ⭐: User preference memory for persistent behaviors. **See: `plugins/TEAMMATE.md`** (CLI: `decapod data teammate`)
...
|------|--------|-------|-----------|-------|------------|--------------|--------------|
| todo | implemented | REAL | `plugins/TODO.md` | user+repo | writes | `decapod todo schema` | store isolation, deterministic rebuild (repo) |
| health | implemented | REAL | `plugins/HEALTH.md` | both | writes | `decapod govern health get` | deterministic proof hooks |
| policy | implemented | REAL | `plugins/POLICY.md` | both | writes | `decapod govern policy riskmap verify` | risk gating, trust analysis |
| cron | implemented | REAL | `plugins/CRON.md` | both | writes | `decapod auto cron schema` | brokered sqlite, audit trail |
| reflex | implemented | REAL | `plugins/REFLEX.md` | both | writes | `decapod auto reflex schema` | brokered sqlite, audit trail |
| watcher | implemented | REAL | `plugins/WATCHER.md` | both | reads | `decapod govern watcher run` | audit trail |
| knowledge | implemented | REAL | `plugins/KNOWLEDGE.md` | both | writes | `decapod data knowledge search` | provenance check |
| teammate | implemented | REAL | `plugins/TEAMMATE.md` | both | writes | `decapod data teammate schema` | user preference persistence |
| archive | implemented | REAL | `plugins/ARCHIVE.md` | both | writes | `decapod data archive verify` | hash matching |
| feedback | implemented | REAL | `plugins/FEEDBACK.md` | both | writes | `decapod govern feedback propose` | non-binding isolation |
| trust | implemented | DEPRECATED | `plugins/TRUST.md` | both | reads | `decapod govern health autonomy` | merged into health |
| context | implemented | REAL | `plugins/CONTEXT.md` | both | writes | `decapod data context audit` | budget gating |
| heartbeat | implemented | DEPRECATED | `plugins/HEARTBEAT.md` | both | reads | `decapod govern health summary` | merged into health |
| docs | implemented | REAL | `core/DECAPOD.md` | N/A | reads | `decapod docs list` | embedded assets |
| db_broker | planned | SPEC | `plugins/DB_BROKER.md` | both | both | (not yet enforced) | planned: "no sqlite opens outside broker" |
...
- `plugins/TODO.md` - TODO subsystem reference
- `plugins/TEAMMATE.md` - Teammate preference memory
