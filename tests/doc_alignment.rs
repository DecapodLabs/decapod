...
fn test_agent_docs_command_contracts_alignment() {
    ...
    let contracts_content = fs::read_to_string(contracts_path)
        .expect("failed to read docs/agent/command-contracts.md");
...
fn test_config_schema_alignment() {
    ...
    let caps_json: serde_json::Value = serde_json::from_slice(&output.stdout)
        .expect("failed to parse capabilities JSON");
