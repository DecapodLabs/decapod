...
#[test]
fn managed_dockerfile_release_pin_accepts_current_debian_or_alpine() {
    let release = expected_managed_decapod_version();
    let debian = format!(
        "# Generated by decapod container profile\n\
         ARG DECAPOD_IMAGE={}\n\
         FROM $DECAPOD_IMAGE\n\
         ARG DECAPOD_VERSION={release}\n\
         RUN echo keep\n",
        expected_managed_decapod_image(release)
    );
    managed_dockerfile_matches_release(&debian, release).expect("debian pin");
    ...
    let alpine = format!(
        "# Generated by decapod container profile\n\
         ...
         FROM $DECAPOD_IMAGE\n\
         ARG DECAPOD_VERSION={release}\n",
        expected_managed_decapod_image_alpine(release)
    );
    managed_dockerfile_matches_release(&alpine, release).expect("alpine pin");
}

#[test]
fn managed_dockerfile_release_pin_rejects_stale_version() {
    let release = expected_managed_decapod_version();
    let stale = "\
# Generated by decapod container profile
ARG DECAPOD_IMAGE=ghcr.io/decapodlabs/decapod:v0.0.0-debian
FROM $DECAPOD_IMAGE
ARG DECAPOD_VERSION=0.0.0
";
    let err = managed_dockerfile_matches_release(stale, release).expect_err("stale pin");
    assert!(
