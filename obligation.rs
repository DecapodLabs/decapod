...
#[derive(Debug, Serialize, Deserialize, Clone)]
pub struct ObligationValidation {
    pub node_id: String,
    pub status: ObligationStatus,
    pub reason: String,
    pub dependencies_valid: bool,
    pub proofs_valid: bool,
    pub commit_valid: bool,
...
pub struct ObligationValidationReport {
    ...
    pub acyclic: bool,
    pub status_summary: Vec<(ObligationStatus, usize)>,
    pub validation_errors: Vec<String>,
    pub nodes_with_errors: Vec<String>,
    pub dependency_errors: Vec<String>,
    pub proof_errors: Vec<String>,
    pub commit_errors: Vec<String>,
...
pub fn get_obligation(store: &Store, id: &str) -> Result<ObligationNode, error::DecapodError> {
    ...
    broker.with_conn(&db_path, "decapod", None, "obligation.get", |conn| {
        ...
        ).map_err(|e| error::DecapodError::RusqliteError(e))
    })
}
...
fn check_proof_satisfied(store: &Store, proof_label: &str) -> Result<bool, error::DecapodError> {
    // This should link to health subsystem.
    // For now, let's look for a claim with this ID that is VERIFIED.
    let broker = DbBroker::new(&store.root);
